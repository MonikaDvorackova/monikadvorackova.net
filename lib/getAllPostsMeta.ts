...
/**
 * Reads all markdown/MDX posts from /posts (same source as GET /api/posts).
 */
...
export function postHasTag(post: PostMeta, tagFromUrl: string): boolean {
  const want = tagFromUrl.trim().toLowerCase();
